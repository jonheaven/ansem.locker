...
function Hooves({ className }: { className?: string }) {
  return (
...
/** Diamond then hooves — same scale, left to right. */
export function DiamondHoovesIcon({ className, size = 'md' }: DiamondHoovesIconProps) {
  const icon = SIZE[size];

  return (
    <span
      className={cn('inline-flex shrink-0 items-center gap-0.5 text-accent', className)}
      aria-hidden
    >
      <Diamond className={icon} strokeWidth={2} />
      <Hooves className={cn(icon, 'w-auto max-w-[1.1em]')} />
    </span>
  );
}
